 ...
 */
// 문제1 실행코드
// const d1 = new Duration('0305', '0502');
// console.log(d1); // {date1: new Date() , date2: new Date()}
// console.log(String(d1)); // 2021/03/05 ~ 2021/05/02;

// 문제 2. 두 개의 Duration인스턴스의 겹치는 날짜를 구해볼꺼임.
function strToDate(str) {
  return new Date(
    new Date().getFullYear(),
    Number(str.slice(0, 2) - 1),
...
/* 두 날짜를 비교하는 법 : >,< 연산자를 사용하면 암묵적으로 valueOf()가 호출되어 초로 숫자가 나오게 됨.
// 1970 1 1 0 0 0 000  ms
...
// 문제3
Duration.prototype.intersection = function (other) {
  if (!this.isIntersect(other)) {
    return null;
  }

  const newStart = Math.max(this.date1.getTime(), other.date1.getTime());
  const newEnd = Math.min(this.date2.getTime(), other.date2.getTime());

  const [, sm, sd] = dateToStr(new Date(newStart)).split('/'); // 2021/03/02
  const [, em, ed] = dateToStr(new Date(newEnd)).split('/');

  return new Duration(`${sm}${sd}`, `${em}${ed}`);
};

// 문제3 실행코드
const d1to2 = d1.intersection(d2);
// d3 도 Duration인스턴스여야 함
console.log(String(d1to2)); // 2021/03/04 ~ 2021/03/05
